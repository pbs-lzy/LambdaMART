...
public class LambdaMARTwithRF {
	public static void main(String[] args) {
		final int numOfTrainingSamples = 6486;
		final int numofTestSamples = 2622;
		final int numOfFeatures = 46;
		// final int numOfTrainingSamples = 19;
		// final int numofTestSamples = 19;
		// final int numOfFeatures = 5;
		final int truncationLevel = 100;
		final int numOfTrees = 1;
		final int numOfLeaves = 2;
		final double learningRate = 0.1;
		final int minSamplesPerLeaves = 1;
		final String trainingPath = "./MQ2008/Fold4/newtrain.txt";
		//final String trainingPath = "../simple.txt";
		final String testPath = "./MQ2008/Fold4/newtest.txt";
		//final String testPath = "./MQ2007/Fold2/newtrain.txt";
		final Boolean useRF = false;
		...
		try {
			BufferedReader br = new BufferedReader(new FileReader(trainingPath));
			String data = null;
		...
		for (int athListWithSameqid = 0; athListWithSameqid < trainingLastIndex.size(); athListWithSameqid++) {
			...
			int smaller = truncationLevel < numOfSameqid ? truncationLevel : numOfSameqid;
		...
		System.out.println("The original average NDCG for training set is " + original_average_NDCG);
		...
		System.out.println("The original average NDCG for test set is " + original_average_NDCG);

		if (useRF) {
			//	random forest
			RandomForest rf = new RandomForest();
			rf.trainingSamples = trainingSamples;
			rf.testSamples = testSamples;
			rf.bootstrap();
			...
			for (int i = 0; i < numOfTrainingSamples; i++) {
				...
				trainingSamples[i].score = 0;
				//System.out.println(trainingSamples[i].label + " " + trainingSamples[i].score);
			}
			...
			System.out.println("RF has finished");
			//	the average training NDCG after RF
			double average_NDCG_RF = 0;
			for (int athListWithSameqid = 0; athListWithSameqid < trainingLastIndex.size(); athListWithSameqid++) {
				int firstIndex = trainingFirstIndex.get(athListWithSameqid);
				int lastIndex = trainingLastIndex.get(athListWithSameqid);
				int numOfSameqid = lastIndex - firstIndex + 1;
			...
			average_NDCG_RF = average_NDCG_RF / trainingFirstIndex.size();
			System.out.println("average training NDCG after RF:" + average_NDCG_RF);
			...
			average_NDCG_RF = average_NDCG_RF / testFirstIndex.size();
			System.out.println("average test NDCG after RF:" + average_NDCG_RF);
		}
		...
		for (int kthtree = 0; kthtree < numOfTrees; kthtree++) {
			...
			System.out.println(kthtree + "th NDCG:" + average_NDCG);
		}
		...
		/*for (int i = 0; i < numOfTrainingSamples; i++) {
			...
			result += "" + trainingSamples[i].label + " ";
			result += "lambda:" + trainingSamples[i].lambda + " ";
			result += "weight" + trainingSamples[i].weight + " score:" + trainingSamples[i].score;
			System.out.println(result);
		}*/
		...
		System.out.println("The number of trees in LambdaMART:" + numOfTrees);
		System.out.println("The number of leaves in LambdaMART:" + numOfLeaves);
